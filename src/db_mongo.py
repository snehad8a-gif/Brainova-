...
def get_db():
    ...
    if DB is None:
        ...
        try:
            ...
            # Default DB name updated to brainova
            db_name = "brainova" 
            if "/" in uri.split("://")[-1]:
                db_name = uri.split("/")[-1].split("?")[0] or "brainova"
            DB = CLIENT[db_name]
...
def init_gamification_db():
    """Ensure user_progress doc exists for Brainova gamification."""
    db = get_db()
    ...
    try:
        ...
        if habit:
            ...
            if habit_logs_df.empty:
                habit_logs_df = pd.DataFrame([log_entry])
