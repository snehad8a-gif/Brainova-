...
BADGES = {
    "first_step": {"name": "First Step", "icon": "👟", "desc": "Complete your first habit on Brainova"},
    ...
    "month_master": {"name": "Monthly Master", "icon": "🏆", "desc": "Achieve a 30-day streak on Brainova"},
}

# --- PURE LOGIC ---

def get_level_info(total_xp):
    """Return the current Brainova level object based on total XP."""
    current = LEVELS[0]
    for lvl in LEVELS:
...
def calculate_xp_gain(streak_current, streak_before):
    """
    Calculate XP earned for a Brainova completion.
    """
...
def check_new_badges(logs_df, habits_df, current_badges):
    """
    Check if any new Brainova badges are unlocked.
    logs_df: All logs
    current_badges: list of badge_ids already unlocked
    """
    new_unlocked = []
    
    # 1. First Step (Brainova Journey Begins)
